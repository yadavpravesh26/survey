	...
	if($start_date != 'all' and $start_date != 'YTD' and $start_date != '3months' and $start_date != '6months'){
		...
		$date_to = $start_date.'-12-31';
	  	...
	  	$sql = "SELECT COUNT(ES.ref_id) as tot_sent,
			   ...
			   WHERE ES.com_id = $com_id $cond GROUP BY ES.add_date ORDER BY ES.add_date ASC";
              ...
              <div class="form-selects padd-sty">
                <div class="form-group">
                  ...
                  <select class="form-control" id="d_c_fliter">
					...
					echo '<option value="YTD" selected>YTD</option>';
					echo '<option value="3months">3 months</option>';
					echo '<option value="6months">6 months</option>'; 
					 for($i = date('Y'); $i >= $join_year[0]; $i--)
					  {
						  echo '<option value="'.$i.'">'.$i.'</option>';
					  }
					  	
						//echo '<option value="'.date('Y-m-d', strtotime('-12 month')).'">1 year</option>';
						echo '<option value="all">Overall</option>';
					?>
                  </select>
...
$(document).ready(function(){
	 ...
	 displayAllRecords(400, 0, 'html', asyncv = false,'','all_survey'); 
	 /**/
...
/*Recent Survey END*/
/*Recent Survey Start*/
function displayAllRecords(lim, off, mode, asyncv = false, s_val, showBy) {
             ...
             $.ajax({
               ...
               success: function(response) {
                 ...
                 if(arr.length > 0){
                     ...
                     $("#loader_message").html('');
					if(mode == 'html'){
					   $("#survey_AllList").html(content);
					 } else {
					   $("#survey_AllList").append(content);
					 }
                 } else {
					 if(mode == 'html'){
					   $("#survey_AllList").html(content);
					 ...
					 if($("#survey_AllList").html().length == 0){
						 $("#loader_message").html('<div class="col-md-12 col-sm-12 col-xs-12"><div class="alert alert-danger"><i class="fa fa-info-circle"></i> <strong>No records found..</strong></div></div>').show();
					 } else{$("#loader_message").html('');}
                 }
